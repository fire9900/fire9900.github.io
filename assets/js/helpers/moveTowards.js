export function moveTowardsPoints(person, destinationPosition, speed) {
...
export function moveTowardsCenters (person, destinationPosition, speed) {
  let distanceToTravelX = destinationPosition.x - person.center.x;
  let distanceToTravelY = destinationPosition.y - person.center.y;

  let distance = Math.sqrt(distanceToTravelX**2 + distanceToTravelY**2);

  if (distance <= speed) {
    // If we're close enough, just move directly to the destination
    person.position.x = destinationPosition.x-person.width/2;
    person.position.y = destinationPosition.y-person.height/2;
  } else {
    // Otherwise, move by the specified speed in the direction of the destination
    let normalizedX = distanceToTravelX / distance;
    let normalizedY = distanceToTravelY / distance;
    ...
    // Recalculate remaining distance after the move
    distanceToTravelX = destinationPosition.x - person.center.x;
    distanceToTravelY = destinationPosition.y - person.center.y;
    distance = Math.sqrt(distanceToTravelX**2 + distanceToTravelY**2);
  }
